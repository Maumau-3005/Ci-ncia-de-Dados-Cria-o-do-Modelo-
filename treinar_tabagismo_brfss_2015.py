...
def obter_modelos(modo: str = "full") -> Dict[str, Any]:
    ...
    return {
        "DT": tree.DecisionTreeClassifier(**parametros),
    }
...
def treinar_e_avaliar(
    ...
    if X_avaliacao is not None and len(X_avaliacao) > 0:
        try:
            ...
            if importancias is not None:
                importancias_ordenadas = sorted(
                    [
                        (coluna, float(importancias[idx]))
                        for idx, coluna in enumerate(colunas_caracteristicas)
                    ],
        ...
        except Exception as erro_permutacao:
            log(f"Não foi possível calcular importâncias de permutação: {erro_permutacao}")
            importancias_ordenadas = []

    resultados_modelo[melhor_modelo]["feature_importances"] = importancias_ordenadas
...
def main() -> None:
    ...
    for nome_modelo in sorted(resultados.keys()):
        ...
        val_txt = formatar_percentual(metrics_map.get("val_acc"))
        test_txt = formatar_percentual(metrics_map.get("test_acc"))
        auc_txt = formatar_percentual(metrics_map.get("test_auc"))
    ...
    print(
        ...
        .format(
            val=formatar_percentual(metricas_melhor_modelo.get("val_acc")),
            test=formatar_percentual(metricas_melhor_modelo.get("test_acc")),
            auc=formatar_percentual(metricas_melhor_modelo.get("test_auc")),
        )
    )
